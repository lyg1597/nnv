...
def runModel(initalCondition, time_bound, time_step, ref_input):
    ...
    #                 f.write(str(param[row, col].item()))
    #                 if col != param.shape[1]-1:
    #                     f.write(', ')
    #         else:
    #             f.write(str(param[row].item()))
    #         if row!=param.shape[0]-1:
    #             f.write(";")
    #         f.write("\n")
    #     f.write(']\n')
    # f.close()
...
def get_transform_information(waypoint: Waypoint) -> Tuple[np.array, float]:
    ...
    # translation_vector[3:] = -1 * np.array([ref_vx, ref_vy])
...
def transform_poly_to_virtual(poly, transform_information):
    ...
    return  poly_out.rotation(i=0, j=1, theta=0)
...
def transform_poly_from_virtual(poly, transform_information):
    ...
    out_poly = poly.rotation(i=0, j=1, theta=0)
    # box = PolyUtils.get_bounding_box(out_poly)
    # out_poly = pc.box2poly(box.T)
    return out_poly.translation(translation_vector)


def transform_mode_from_virtual(waypoint: Waypoint, transform_information):
    point = waypoint.mode_parameters
    sc = 0 #-1 * transform_information[1]
    translation_vector = -1 * transform_information[0]
    xs1 = point[0]  # x_i
    ys1 = point[1]  # y_i
    xd1 = point[3]
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        y_init = np.random.uniform(0.4,0.5)
        theta_init = np.random.uniform(1.57, 1.58)
        trace = TC_Simulate("[20.4142135623735;0.4142135623721741;20.4142135623735;10.4142135623721668]", [x_init,y_init,theta_init], 10)
